(function(){
  ...
  const nativeFaceID=()=>!!(window.__MONETA_NATIVE_IOS__ && window.MonetaNative?.faceID);

  const biometricStatus=async()=>{
    if(nativeFaceID()){
      try{
        const status=await window.MonetaNative.faceID('status','');
        return {available:!!status?.available,reason:status?.available?null:(status?.reason||'native')};
      }catch{return {available:false,reason:'native'};}
    }
  ...
  const biometricError=status=>{
    ...
    if(status.reason==='native')return 'Face ID сейчас недоступен. Проверь, что на iPhone настроены Face ID и код-пароль.';
  ...
  const registerBiometric=async(profile)=>{
    ...
    if(nativeFaceID()){
      const ok=await window.MonetaNative.faceID('authenticate','Подтверди Face ID, чтобы включить быстрый вход в Кошелёк');
      if(!ok)throw new Error('Не удалось подтвердить Face ID');
      S.mutate(p=>{p.biometricCredentialId='native-faceid';p.settings.biometrics=true;});
      return true;
    }
  ...
  const authenticateBiometric=async(profile)=>{
    ...
    if(nativeFaceID()){
      return !!(await window.MonetaNative.faceID('authenticate','Войди в Кошелёк с Face ID'));
    }
